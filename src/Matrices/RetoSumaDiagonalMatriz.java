...
for (int i = 0; i < matrizSumaDiagonal.length; i++) {
    for (int j = 0; j < matrizSumaDiagonal[i].length; j++) {
        ...
        if (i==j) {
        System.out.println("Suma de todas las matrices de manera diagonal primera salida : "+(sumaDiagonal+=matrizSumaDiagonal[i][j]));
        }
}
}
...
pero para la maquina o compilacion es menos optimizable*/{
    System.out.println("Suma de todas las matrices de manera diagonal en contralado: "+(sumaDiagonalContralado+=matrizSumaDiagonal[k][l]));
}
